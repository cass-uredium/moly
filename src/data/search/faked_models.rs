    ...
    models
        ...
        .map(|model| {
            ...
            if model.size.is_empty() {
                new_model.size = format!("{}B", rng.random_range(1..10));
            };

            if model.requires.is_empty() {
                new_model.requires = match rng.random_range(0..3) {
                    0 => "4GB+ RAM".to_string(),
                    1 => "8GB+ RAM".to_string(),
            ...
            if model.architecture.is_empty() {
                new_model.architecture = match rng.random_range(0..3) {
                    0 => "Mistral".to_string(),
            ...
            if model.like_count == 0 {
                new_model.like_count = rng.random_range(1..1000);
            };

            if model.download_count == 0 {
                new_model.download_count = rng.random_range(0..10000);
            };
            ...
            let new_files: Vec<File> = model
                ...
                .map(|file| {
                    ...
                    if file.size.is_empty() {
                        new_file.size = rng.random_range(100000000..999999999).to_string();
                    };

                    new_file.featured = rng.random_bool(0.15);
